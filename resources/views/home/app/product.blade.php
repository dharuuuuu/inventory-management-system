  ...
  <section class="mx-auto max-w-screen-2xl px-4 md:px-8">
    ...
    <div class="mb-8 flex flex-wrap justify-between md:mb-16">
        ...
        @foreach ($produk as $data)
            <div class="w-full">
                <a href="/produk/{{ $data->slug }}" class="block relative h-48 rounded overflow-hidden">
                @if ($data->image != NULL)
                <img class="object-cover object-center w-full h-full block" src="{{ url('storage') }}/{{ $data->image }}" alt="Gambar {{ $data->name }}">
                @else
                <img class="object-cover object-center w-full h-full block" src=https://dummyimage.com/420x260/ddffdd/007300&text=Image+Not+Found" alt="Image Not Found">
                @endif
                </a>
                <div class="mt-4">
                {{-- <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ $data->category_id }}</h3> --}}
                <h2 class="text-gray-900 title-font text-lg font-medium">{{ $data->name }}</h2>
                <p class="mt-1">Rp. {{ $data->selling_price }}</p>
                </div>
            </div>
        @endforeach
        </div>
